...
func (s *DispatchServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if len(bodyBytes) > 0 && !strings.HasPrefix(r.URL.Path, getConfig().Admin.Route) {
...
func (s *DispatchServer) handleAdminLogin(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	ip := clientIP(r)
	if locked, remain := checkAdminLocked(ip); locked {
		sendJSON(w, map[string]interface{}{"ok": false, "error": L("admin_login_locked", remain)})
		return
	}
	...
	resp := map[string]interface{}{"ok": true, "token": token, "username": req.Username}
	if isDefaultAdminCredential(creds) {

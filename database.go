...
// Admin login failure lock (per client IP)
const (
	adminMaxFails = 3
	adminLockMin  = 10
)
...
var (
	...
	adminFails    = map[string]adminFailInfo{}
	adminLockOuts = map[string]time.Time{}
)

type adminFailInfo struct {
	fails       int
	lastAttempt time.Time
}

func checkAdminLocked(ip string) (bool, string) {
	adminFailMu.Lock()
	defer adminFailMu.Unlock()
	if until, ok := adminLockOuts[ip]; ok {
		if time.Now().Before(until) {
			remain := int(time.Until(until).Minutes())
			if remain < 1 {
				remain = 1
			}
			return true, fmt.Sprintf("%d", remain)
		}
		delete(adminLockOuts, ip)
...
func recordAdminFail(ip string) {
	...
	if f.fails >= adminMaxFails {
		adminLockOuts[ip] = time.Now().Add(adminLockMin * time.Minute)
		delete(adminFails, ip)
		return
	}
	adminFails[ip] = f
...
func resetAdminFails(ip string) {
	adminFailMu.Lock()
	defer adminFailMu.Unlock()
	delete(adminFails, ip)
}
...
func isDefaultAdminCredential(creds *AdminCredentials) bool {
	if creds == nil {
		return false
	}
	if creds.Username == defaultAdminUser {
		return true
	}
	return checkPassword(defaultAdminPass, creds.PasswordHash)
}
